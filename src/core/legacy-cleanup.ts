...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const dir of result.deletedDirs) {
      lines.push(`  ✓ 已删除 ${dir}/（已被 /opsx:* 替代）`);
    }

    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ 已从 ${file} 中移除 OpenSpec 标记`);
    }
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push('');
  lines.push('OpenSpec 现已使用 Agent 技能，这是编码代理的新标准。');
  lines.push('这简化了您的设置，同时保持一切正常运行。');
  lines.push('');

  // Section 1: Files to remove (no user content to preserve)
  if (removals.length > 0) {
    lines.push(chalk.bold('待删除的文件'));
...
export function formatProjectMdMigrationHint(): string {
  const lines: string[] = [];
  lines.push(chalk.yellow.bold('需要您的注意'));
  ...
  lines.push(chalk.dim('    上下文。这包含在每个 OpenSpec 请求中，比旧的'));
  lines.push(chalk.dim('    project.md 方式更可靠。'));
  lines.push('');
  lines.push(chalk.dim('    审查 project.md，将有用内容移至 config.yaml 的 context'));
  lines.push(chalk.dim('    部分，准备好后删除该文件。'));
  return lines.join('\n');
}
