...
export async function instructionsCommand(
  ...
  try {
    ...
    if (!artifact) {
      ...
      const validIds = context.graph.getAllArtifacts().map((a) => a.id);
      throw new Error(
        `工件 '${artifactId}' 在 schema '${context.schemaName}' 中未找到。有效的工件：\n  ${validIds.join('\n  ')}`
      );
    }
...
export function printInstructionsText(instructions: ArtifactInstructions, isBlocked: boolean): void {
  ...
  if (context) {
    ...
    console.log('<!-- 这是为您提供的背景信息。请勿在输出中包含此内容。 -->');
    console.log(context);
    console.log('</project_context>');
    console.log();
  }
  ...
  if (rules && rules.length > 0) {
    console.log('<rules>');
    console.log('<!-- 这些是您需要遵循的约束。请勿在输出中包含此内容。 -->');
  ...
  // Output location
  console.log('<output>');
  console.log(`输出位置：${path.join(changeDir, outputPath)}`);
  console.log('</output>');
  console.log();
  ...
  console.log('<!-- 使用此结构作为输出文件的模板。填写各章节内容。 -->');
...
export async function generateApplyInstructions(
  ...
  if (missingArtifacts.length > 0) {
    state = 'blocked';
    instruction = `尚无法应用此变更。缺少工件：${missingArtifacts.join(', ')}。\n请使用 openspec-continue-change 技能先创建缺失的工件。`;
    ...
    const tracksFilename = path.basename(tracksFile);
    state = 'blocked';
    instruction = `${tracksFilename} 文件存在但不包含任何任务。\n请向 ${tracksFilename} 添加任务或使用 openspec-continue-change 重新生成。`;
    ...
    state = 'ready';
    instruction = schemaInstruction?.trim() ?? '所有必需工件已完成。请继续实施。';
  } else {
    state = 'ready';
    instruction = schemaInstruction?.trim() ?? '阅读上下文文件，逐步完成待办任务，完成后标记为已完成。\n如遇到阻碍或需要澄清，请暂停。';
  }
...
export function printApplyInstructionsText(instructions: ApplyInstructions): void {
  ...
  if (state === 'blocked' && missingArtifacts) {
    console.log('### ⚠️ 已阻塞');
    console.log();
    console.log(`缺少工件：${missingArtifacts.join(', ')}`);
    console.log('请使用 openspec-continue-change 技能先创建这些工件。');
    console.log();
  }
